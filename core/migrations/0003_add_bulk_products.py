# Generated by Django 6.0.5 on 2026-06-03 07:21

from django.db import migrations

def add_all_products(apps, schema_editor):
    Product = apps.get_model('core', 'Product')
    Category = apps.get_model('core', 'Category')
    ProductImage = apps.get_model('core', 'ProductImage')

    cat, _ = Category.objects.get_or_create(name="Apparel")

    product_data = [
        {"name": "Black Slim Jeans", "price": 49.99, "stock": 20, "filename": "black_jeans.avif", "desc": "Premium slim-fit black denim."},
        {"name": "Black T-Shirt", "price": 19.99, "stock": 50, "filename": "black_t-shirts.avif", "desc": "Classic black cotton tee."},
        {"name": "Check Shirt", "price": 25.00, "stock": 15, "filename": "chex_shirts.avif", "desc": "Grey and black checkered flannel."},
        # ... (add the rest of your items here)
    ]

    for item in product_data:
        product = Product.objects.create(category=cat, name=item['name'], description=item['desc'], price=item['price'], stock=item['stock'])
        ProductImage.objects.create(product=product, image_filename=item['filename'], alt_text=f"{item['name']} front view")

class Migration(migrations.Migration):
    # Ensure this points to the migration that added 'image_filename' (Step 2)
    dependencies = [('core', '0002_remove_productimage_image_url_and_more')] 
    operations = [migrations.RunPython(add_all_products)]
